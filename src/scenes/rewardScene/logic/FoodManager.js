var FoodManager = cc.Class.extend({
    ...
    init: function () {
        this.removeAll();
        this._pattern = 1;
        this._patternPosY = cc.director.getWinSize().height - RewardConstants.GAME_AREA_TOP_BOTTOM;
        ...
        RewardSceneProps.user.coffee = RewardSceneProps.user.mushroom = 0;
    },
    removeAll: function () {
    ...
    _setFoodPattern: function (elapsed) {
        ...
        if (this._patternChangeDistance > 0) {
            this._patternChangeDistance -= RewardSceneProps.user.heroSpeed * elapsed;
        } else {
    ...
    _createFoodPattern: function (elapsed) {
        ...
        if (this._patternGapCount < this._patternGap) {
            this._patternGapCount += RewardSceneProps.user.heroSpeed * elapsed;
            ...
            switch (this._pattern) {
                case 1:
                    ...
                    if (Math.random() > 0.9) {
                        ...
                        this._patternPosY = Math.floor(Math.random() * (winSize.height - 2 * RewardConstants.GAME_AREA_TOP_BOTTOM)) + RewardConstants.GAME_AREA_TOP_BOTTOM;
                    }
                ...
                case 3:
                    ...
                    if (this._patternDirection === 1 && this._patternPosY < RewardConstants.GAME_AREA_TOP_BOTTOM) {
                        this._patternDirection = -1;
                    }
                    else if (this._patternDirection === -1 && this._patternPosY > winSize.height - RewardConstants.GAME_AREA_TOP_BOTTOM) {
                        this._patternDirection = 1;
                    }

                    if (this._patternPosY <= winSize.height - RewardConstants.GAME_AREA_TOP_BOTTOM && this._patternPosY >= RewardConstants.GAME_AREA_TOP_BOTTOM) {
                        item = Item.create(Math.ceil(Math.random() * 5));
                        item.x = winSize.width + item.width;
                        item.y = this._patternPosY;
                        this._itemsToAnimate.push(item);
                        ...
                        this._patternPosY = winSize.height - RewardConstants.GAME_AREA_TOP_BOTTOM;
                    }
                ...
                case 10:
                    ...
                    // Set a new random position for the item, making sure it's not too close to the edges of the screen.
                    this._patternPosY = Math.floor(Math.random() * (winSize.height - 2 * RewardConstants.GAME_AREA_TOP_BOTTOM)) + RewardConstants.GAME_AREA_TOP_BOTTOM;
                    item = Item.create(RewardConstants.ITEM_TYPE_COFFEE);
                    item.x = winSize.width + item.width;
                    item.y = this._patternPosY;
                    this._itemsToAnimate.push(item);
                ...
                case 11:
                    ...
                    this._patternPosY = Math.floor(Math.random() * (winSize.height - 2 * RewardConstants.GAME_AREA_TOP_BOTTOM)) + RewardConstants.GAME_AREA_TOP_BOTTOM;
                    item = Item.create(RewardConstants.ITEM_TYPE_MUSHROOM);
                    item.x = winSize.width + item.width;
                    item.y = this._patternPosY;
                    this._itemsToAnimate.push(item);
    ...
    _animateFoodItems: function (hero, elapsed) {
        ...
        for (var i = this._itemsToAnimate.length - 1; i >= 0; i--) {
            ...
            if (item) {
                ...
                if (RewardSceneProps.user.mushroom > 0 && item.type <= RewardConstants.ITEM_TYPE_5) {
                    ...
                    item.x -= RewardSceneProps.user.heroSpeed * elapsed;
                }

                // If the item passes outside the screen on the left, remove it (check-in).

                if (item.x < -80 || RewardSceneProps.gameState === RewardConstants.GAME_STATE_OVER) {
                    this._itemsToAnimate.splice(i, 1);
                    ...
                    if (heroItem_sqDist < 5000) {
                        ...
                        if (item.type <= RewardConstants.ITEM_TYPE_5) {
                            // RewardSceneProps.user.score += item.type;
                            Game.character.score++;//得分增加
                        } else if (item.type === RewardConstants.ITEM_TYPE_COFFEE) {
                            // If hero drinks coffee, add up the score.
                            RewardSceneProps.user.score += 1;

                            // How long does coffee power last? (in seconds)
                            RewardSceneProps.user.coffee = 5;
                            RewardEffect.showCoffeeEffect(this._gameScene);
                        } else if (item.type === RewardConstants.ITEM_TYPE_MUSHROOM) {
                            // If hero eats a mushroom, add up the score.
                            RewardSceneProps.user.score += 1;

                            // How long does mushroom power last? (in seconds)
                            RewardSceneProps.user.mushroom = 4;
                            RewardEffect.showMushroomEffect(this._gameScene);
                        }

                        // Create an eat particle at the position of the food item that was eaten.
                        RewardEffect.showEatEffect(this._gameScene, item.x, item.y);
                        Sound.playEat();
